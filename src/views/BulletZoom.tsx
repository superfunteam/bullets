...
export function BulletZoom({
  ...
  const addChild = async () => {
    ...
    if (!t) return;
    await createBullet({ title: t, parentId: bulletId, clientId: bullet.clientId });
    setChildTitle('');
  };
          ...
          {liveShots.length > 0 && (
            <section className="mt-8">
              ...
              <div className="space-y-2">
                {liveShots.map(s => (
                  <div key={s.id} className="meta flex justify-between px-1 text-[var(--ink-2)]">
                    <span>
            ...
            <div className="flex flex-wrap gap-2">
              {HORIZONS.map((h: Horizon) => (
                <motion.button
                  ...
                  whileTap={{ scale: 0.94 }}
                  transition={snap}
                  onClick={() => void (h === 'shelf' ? shelve(bullet.id) : setHorizon(bullet.id, h))}
                >
